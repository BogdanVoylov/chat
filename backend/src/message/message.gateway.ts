import {
    WsResponse,
    MessageBody,
    SubscribeMessage,
    WebSocketGateway,
    WebSocketServer,
} from '@nestjs/websockets';
import { from, Observable } from 'rxjs';
import { map } from 'rxjs/operators';
import { Server } from 'socket.io';
import { Logger } from '@nestjs/common';

import { makeid } from 'src/utils/stringUtils'

type Message = {
    author: string,
    recipient: string,
    text: string,
    timestamp: number
}

type User = {
    online: boolean,
    name: string,
    avatar: string
}

class Dialog {
    authors: string[]
    messages: Message[]
    constructor(authors: string[]) {
        this.authors = authors;
        this.messages = []
    }

    public addMessage(m: Message) {
        this.messages.push(m)
    }

    public isSuitable(u1: string, u2: string): boolean {
        return this.authors.includes(u2) && this.authors.includes(u1)
    }
}

@WebSocketGateway()
export class MessageGateway {
    @WebSocketServer()
    server: Server;

    private userIdLen = 10;

    private dialogs: Dialog[] = []
    private users: User[] = []

    private logger: Logger = new Logger('MessageGateway');


    @SubscribeMessage('message')
    async addMessage(@MessageBody() message: Message): Promise<Message> {
        this.logger.log(`message: ${JSON.stringify(message)}`);
        const dialog = this.findDialog(message.author, message.recipient)
        if (dialog != null) {
            dialog.addMessage(message);

        } else {
            const dialog = new Dialog([message.author, message.recipient]);
            dialog.addMessage(message);
            this.dialogs.push(dialog);
        }
        this.server.emit(message.recipient, message)
        this.server.emit(message.author, message)
        return message;
    }

    @SubscribeMessage('dialog')
    async getDialog(@MessageBody() [name1, name2]): Promise<Message[]> {
        this.logger.log(`Dialog request: ${name1} ${name2}`);
        return this.findDialog(name1, name2)?.messages ?? [];
    }

    @SubscribeMessage('users')
    async newUser(@MessageBody() userName: string | null): Promise<{ users: User[], user: User }> {
        let name = userName ?? makeid(this.userIdLen);
        let avatar = "data:image/png;base64,"
        let user: User = { name, avatar, online: true };
        this.logger.log(`user: ${name} ${makeid(this.userIdLen)}`);
        if (!this.users.find(u => u.name === name)) {
            this.users.push(user)
        }

        this.server.emit('users', this.users);
        return { users: this.users, user };
    }

    private findDialog(u1: string, u2: string): Dialog {
        return this.dialogs.find(d => d.isSuitable(u1, u2))
    }


}